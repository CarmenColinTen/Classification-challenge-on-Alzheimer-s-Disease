...
train_data <- read.csv('ADMCItrain.csv')
...
lasso.coef <- predict(lasso.mod, s = lasso.cv$lambda.min, type = "coefficients")[1:6, ]
...
mypredictions <- read.csv('75741_ADMCIres.csv')
mypred <- mypredictions[, 2]
mylabel <- mypredictions[, 4]

# Convert the predictions to numeric
mypred <- as.numeric(as.character(mypred))

# Convert the labels to numeric
mylabel <- ifelse(mylabel == "CTL", 0, 1)

# Compute the AUC
auc <- pROC::auc(mypred, mylabel)
